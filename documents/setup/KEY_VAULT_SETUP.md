...
|--------|-------|---------|
| `azure-openai-endpoint` | `https://your-openai-resource.openai.azure.com/` | Azure OpenAI service endpoint |
| `azure-deployment-name` | `NextGenGPT` | GPT-5.2 model deployment |
| `azure-api-version` | `2024-12-01-preview` | OpenAI API version |
| `azure-subscription-id` | `your-subscription-id` | Your Azure subscription |
| `azure-resource-group` | `your-resource-group` | Resource group name |
| `sql-server` | `your-sql-server.database.windows.net` | SQL Server hostname |
...
**List all secrets:**
```bash
az keyvault secret list --vault-name your-keyvault-name --query "[].name"
```
...
az monitor metrics alert create \
  ...
  --resource-group your-resource-group \
  --scopes $(az keyvault show --name your-keyvault-name --query id -o tsv) \
...
az keyvault purge --location westus2 --name your-keyvault-name
```
