...
az postgres flexible-server create \
  ...
  --admin-password 'YourStrongP@ssword!' \
...
az container create \
  ...
  --environment-variables \
    ...
    POSTGRES_PASSWORD='YourStrongP@ssword!' \
  --dns-name-label nextgen-postgres
```
...
cat > .env << 'EOF'
# PostgreSQL
POSTGRES_HOST=nextgen-postgres.postgres.database.azure.com
POSTGRES_PORT=5432
POSTGRES_DB=ApplicationsDB
POSTGRES_USER=adminuser
POSTGRES_PASSWORD=YourStrongP@ssword!
...
# Or use DATABASE_URL
# DATABASE_URL=postgresql://adminuser:YourStrongP@ssword!@nextgen-postgres.postgres.database.azure.com:5432/ApplicationsDB?sslmode=require
...
az keyvault secret set --vault-name nextgen-agents-kv \
  --name postgres-host --value nextgen-postgres.postgres.database.azure.com
...
az keyvault secret set --vault-name nextgen-agents-kv \
  --name postgres-password --value 'YourStrongP@ssword!'
```
...
```bash
# Test with psql CLI
psql "postgresql://adminuser:password@nextgen-postgres.postgres.database.azure.com:5432/ApplicationsDB?sslmode=require"

# Or with Python
python3 << 'EOF'
import psycopg
conn = psycopg.connect(
    "host=nextgen-postgres.postgres.database.azure.com "
    "port=5432 dbname=ApplicationsDB user=adminuser "
    "password='YourStrongP@ssword!' sslmode=require"
)
