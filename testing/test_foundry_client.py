...
def test_derive_model_endpoint_strips_projects_and_rewrites_domain():
    ...
    raw = "https://myfoundry.services.ai.azure.com/api/projects/myproject"
    client = FoundryClient(endpoint=raw)
    # internal model endpoint should be cognitiveservices host only
    assert client._model_endpoint == "https://myfoundry.cognitiveservices.azure.com"
    ...
    raw3 = "https://myfoundry.services.ai.azure.com/api/projects/myproject/applications/SchoolAccessIndex/protocols/openai/responses"
    client3 = FoundryClient(endpoint=raw3)
    assert client3._model_endpoint == "https://myfoundry.cognitiveservices.azure.com"

    # if given an already-correct URL it should be preserved
    raw2 = "https://myfoundry.cognitiveservices.azure.com/"
    client2 = FoundryClient(endpoint=raw2)
    assert client2._model_endpoint == "https://myfoundry.cognitiveservices.azure.com"
...
def test_create_completion_uses_normalized_url(monkeypatch):
    ...
    assert called['url'].startswith("https://myfoundry.cognitiveservices.azure.com/openai/deployments/mydep")
    ...
    called.clear()
    client_suffix = FoundryClient(endpoint="https://myfoundry.services.ai.azure.com/api/projects/myproject/applications/SchoolAccessIndex/protocols/openai/responses")
    resp2 = client_suffix.chat.create(model="moana", messages=[{"role": "user", "content": "hello"}])
    assert called['url'].startswith("https://myfoundry.cognitiveservices.azure.com/openai/deployments/moana")
