...
# Verify deployment
az webapp show --resource-group NextGen_Agents --name nextgen-agents-app --query defaultHostName
```
...
```bash
# View live logs
az webapp log tail --resource-group NextGen_Agents --name nextgen-agents-app

# Download log files
az webapp log download --resource-group NextGen_Agents --name nextgen-agents-app

# Get web app status
az webapp show --resource-group NextGen_Agents --name nextgen-agents-app \
...
```bash
az appservice plan update --name NextGen-AppServicePlan \
  --resource-group NextGen_Agents --sku B3
```

### Scale Out (Multiple Instances)
```bash
az appservice plan update --name NextGen-AppServicePlan \
  --resource-group NextGen_Agents --number-of-workers 2
```
...
az webapp deployment log show --resource-group NextGen_Agents --name nextgen-agents-app
